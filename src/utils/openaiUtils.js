...
export async function summarizeAndExtractKeywords(text) {
  ...
  const keywordResponse = await openai.createChatCompletion({
    ...
    messages: [
      {
        role: 'system', content: `
        You are an assistant that extracts keywords for trading-related content to optimize search indexing, smantic search, AI search. 
        ...
        Keywords should be listed, separated by a comma, maxium 12 keywords:` },
      { role: 'user', content: text }
    ],
    max_tokens: 50,
  });
